...
/** Every position holds a tile: the occupancy a full board is asked about. */
...
/**
 * The predicate under these three is the SHIPPED `isFree`, not a transcription
 * of it. `layout.test.ts` used to carry its own copy, because this file's
 * playability invariant needed a freedom rule before `freedom.ts` existed and
 * its docblock said in writing to delete it the day one landed. It has: two
 * copies of a rule are two chances to disagree about it, and the disagreement
 * would have shown up here, in the invariant that trusts it.
 */
describe("freedom on a full turtle", () => {
  const free = LAYOUT.filter((_position, index) => isFree(index, WHOLE_BOARD));
