...
describe("generala scorecard: an open box the acting seat may write is a control that says what it is worth", () => {
  ...
  it("refuses the escalera al as, because the engine does", () => {
    ...
    // idea of a straight shows 20 here — and would offer a control with it,
    // because a box worth something is always writable.
    expect(planilla.locked("escalera", 0)?.textContent?.trim()).toBe("0");
    expect(planilla.button("escalera", 0)).toBeNull();
  });
  ...
  it("keeps a box you may still take from one you may not, and both from one already spent", () => {
    ...
    const crossable = planilla.button("generala", 0);
    ...
    expect(crossable?.textContent?.trim()).toBe("0");
    ...
    expect(getComputedStyle(crossable!).textDecorationLine).toBe("none");
    ...
    expect(getComputedStyle(crossable!).backgroundColor).not.toBe(getComputedStyle(written).backgroundColor);
    expect(getComputedStyle(worthNothingAndLocked!).backgroundColor).not.toBe(getComputedStyle(crossable!).backgroundColor);
    expect(Number(getComputedStyle(worthNothingAndLocked!).opacity)).toBeLessThan(Number(getComputedStyle(crossable!).opacity));
  });
