 ...
 * draw enters this game, for either of the two shapes the ruleset needs. The
 ...
 *
 * THE TWO SHAPES (design D3's own data-flow table):
 *
 ...
 *
 * NEITHER SHAPE APPLIES ANYTHING. This function only materializes DATA, the
 * same split `truco-module/src/deal.ts`'s own `createSystemActionRequester`
 * already draws between "the system decided what to deal" and "the module's
 * `applyAction` decided whether that deal may be applied right now". A future
 * `applyAction` (work unit C3, the first seat-count registration) is where
 * `action.playerId !== SYSTEM_ACTOR_ID` must be refused — `deal.ts`'s own
 * docblock records the exact cost of forgetting that check once, for a
 ...
 * D3 shapes above, and nothing else. A future `applyAction` (work unit C3)
 * narrows on `.type` the same way `truco-module/src/index.ts` narrows its own
 * `TrucoModuleAction` union on `"start-hand"`.
 */
 ...
 * `bidding` offers a SEATED PLAYER's action (`legal-actions.ts`, work unit
 * B2) and `showdown` resolves deterministically the instant the doubt that
 * triggered it is applied (`showdown.ts`'s own `resolveShowdown`, work unit
 * B4) — NEITHER needs a system action from this function, so both fall
 * through to the final `return null` with zero draws, proven the same way.
 */
export function requestMentirosoSystemAction(state: MatchState, rng: RandomSource): MentirosoSystemAction | null {
  ...
  // `bidding` and `showdown` — see this function's own docblock for why
  // neither needs entropy from here.
  return null;
}
