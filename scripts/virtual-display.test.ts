...
import { displayIsServed, resolveDisplayRedirect, socketPathFor } from "./virtual-display.mjs";
...
describe("resolveDisplayRedirect", () => {
  ...
  describe("outside linux", () => {
    it("touches nothing on darwin", () => {
      const decision = resolveDisplayRedirect({ platform: "darwin", env: { PATH: "/usr/bin" }, displayAnswers: socketUp });

      expect(decision).toEqual({ action: "none" });
    });

    it("touches nothing on win32, whatever the socket probe claims", () => {
      const decision = resolveDisplayRedirect({ platform: "win32", env: { PATH: "C:\\bin" }, displayAnswers: socketUp });
  ...
  describe("under CI", () => {
    it("touches nothing, because the browser project runs headless there", () => {
      const decision = resolveDisplayRedirect({ platform: "linux", env: { ...waylandEnv, CI: "1" }, displayAnswers: socketUp });

      expect(decision).toEqual({ action: "none" });
    });

    it("accepts any truthy CI value, the way vitest itself does", () => {
      const decision = resolveDisplayRedirect({ platform: "linux", env: { ...waylandEnv, CI: "true" }, displayAnswers: socketUp });
    ...
    it('still redirects when CI is "", which std-env does not count as CI', () => {
      const decision = resolveDisplayRedirect({ platform: "linux", env: { ...waylandEnv, CI: "" }, displayAnswers: socketUp });

      expect(decision.action).toBe("use");
    });

    it('still redirects when CI is "false", which std-env does not count as CI either', () => {
      const decision = resolveDisplayRedirect({ platform: "linux", env: { ...waylandEnv, CI: "false" }, displayAnswers: socketUp });
  ...
  describe("already redirected (the wrapper's explicit marker)", () => {
    it("stands down when the wrapper's marker is present", () => {
      ...
      const decision = resolveDisplayRedirect({ platform: "linux", env: marked, displayAnswers: socketUp });
    ...
    it("treats the wrapper's actual child environment as already handled", () => {
      ...
      const decision = resolveDisplayRedirect({ platform: "linux", env, displayAnswers: socketUp });
    ...
    it("REDIRECTS a genuine X11 desktop session — its shape proves nothing about the wrapper", () => {
      ...
      const decision = redirecting(resolveDisplayRedirect({ platform: "linux", env: nativeX11, displayAnswers: socketUp }));
  ...
  describe("linux, unsanitized session, persistent display already up", () => {
    it("reuses it rather than spawning a second server", () => {
      const decision = redirecting(resolveDisplayRedirect({ platform: "linux", env: waylandEnv, displayAnswers: socketUp }));
  ...
  describe("linux, unsanitized session, no persistent display yet", () => {
    it("asks for a spawn first, against the same display", () => {
      const decision = redirecting(resolveDisplayRedirect({ platform: "linux", env: waylandEnv, displayAnswers: socketDown }));
  ...
  describe("the sanitized environment, on both redirecting actions", () => {
    it.each([
      ["use", socketUp],
      ["spawn", socketDown],
    ] as const)("points DISPLAY at :99 and removes the compositor from reach (%s)", (action, displayAnswers) => {
      const decision = redirecting(resolveDisplayRedirect({ platform: "linux", env: waylandEnv, displayAnswers }));
    ...
    it("leaves the caller's env object untouched", () => {
      resolveDisplayRedirect({ platform: "linux", env: waylandEnv, displayAnswers: socketUp });
...
/**
 * THE STALE SOCKET, which is what `resolveDisplayRedirect` cannot see on its
 * own and what hung this suite five times in one session — twice on a clean
 * tree, looking exactly like a flaky browser test.
 *
 * `socketPathFor`'s own comment had already predicted it: "probe the wrong
 * path and the shim ... trusts a display that is not there." Nothing asserted
 * it, so the caller kept answering with `existsSync` and a dead `Xvfb`'s
 * leftover socket kept reading as a live display.
 */
describe("displayIsServed", () => {
  const up = () => true;
  const down = () => false;

  it("a socket with a server behind it is served", () => {
    expect(displayIsServed({ socketExists: up, serverHoldsDisplay: up }, ":99")).toBe(true);
  });

  it("A STALE SOCKET IS NOT SERVED -- the case that hung the suite", () => {
    expect(displayIsServed({ socketExists: up, serverHoldsDisplay: down }, ":99")).toBe(false);
  });

  it("no socket at all is not served, whatever the process table says", () => {
    expect(displayIsServed({ socketExists: down, serverHoldsDisplay: up }, ":99")).toBe(false);
    expect(displayIsServed({ socketExists: down, serverHoldsDisplay: down }, ":99")).toBe(false);
  });

  it("asks about the display it was given, not a fixed one", () => {
    const seen: string[] = [];
    displayIsServed({ socketExists: (d: string) => (seen.push(d), true), serverHoldsDisplay: (d: string) => (seen.push(d), true) }, ":42");
