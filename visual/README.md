...
1. Run the SPECIFIC file whose baseline needs to change, scoped by name —
   ...
   ```
   Never write a baseline with `pnpm test:visual:host --update`: it would
   bake your own machine's rasterizer into the repo, which is the exact
   problem the container exists to remove.
...
disagree, and a baseline written from it carries your machine's rasterizer
into the repo.
