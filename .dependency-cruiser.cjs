...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: "error",
      comment:
        "dice-ui is pure L0 and must not depend on any other workspace package. It is the Generala dice and cup AS PIECES — geometry, generated SVG faces, the cube's resting-pose table, the credit-free status a functional pip needs no licence for — and it must not know that a Generala engine exists, exactly like mahjong-tile-ui must not know a solitaire exists. Its own rule rather than a `*-ui` glob, for the identical reason mahjong-tile-ui's own rule states: the existing glob is `packages/games/*-ui`, which is L1 game presentation and MAY import its engine; this package sits at packages/ root beside spanish-deck-ui and mahjong-tile-ui and is one tier lower. THE ENGINE AND THE MODULE NOW EXIST (packages/games/generala-engine, packages/games/generala-module), so this rule stopped being a promise about packages nobody had written and became the thing that keeps their rules out of the props: generala-engine declares its OWN DieFace rather than importing this package's, and the two unions meet only in generala-ui, which this rule does not constrain. The rule bytes below are unchanged — what expired was the sentence, not the fence.",
